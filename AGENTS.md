...
- **Reaching the Host from Inside Jail**: Use `host.containers.internal` (recommended) or `host.docker.internal` (docker compatibility alias). Both resolve to the host gateway IP (`169.254.1.2` on podman; varies on docker but same semantic). These hostnames are automatically added to `/etc/hosts` by both podman and docker, and route traffic to the host machine. Useful for agents that need to access host services (e.g., pull from host git servers, reach a development API running on the host). **Works on both podman and docker.**
