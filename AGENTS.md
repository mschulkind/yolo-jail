...
- **Nested Containers (Podman-in-Podman)**: The jail image includes `podman`, `nix`, `fuse-overlayfs`, and `slirp4netns` for nested container operations. Use `--privileged` on the outer container. Inner containers must use `--net=host` (netavark/nftables not available inside). This enables building nix images inside a jail and running containers within containers.
